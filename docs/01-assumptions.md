...
- **The committed default registry is three in-process mocks**: `SOURCE_A`, `SOURCE_B`, `SOURCE_C` are `MOCK`-type entries in `app.sources.registry` (`docs/adr/011-config-driven-source-registry.md`), chosen so the demo and E2E suite stay deterministic and offline. The registry also supports real `KAFKA`, `S3`, and `HTTP` transports — those are not mocks once an entry is configured against them. Timeouts apply per-transport and bounded backoff applies via `IngestionService`'s retry wrapper for all transports; circuit breakers and bulkheads are not implemented (they are evolution, not an assumption of current behaviour).
- **Source Heterogeneity**: It is assumed that different sources provide data in different formats and use different naming conventions for the same concepts (e.g., `amount` vs `value`). Normalization happens at the source boundary via a per-source `SourceNormalizer` strategy, selected by the registry entry's `normalizer` key — independent of which transport fetched the raw record.
...
- **Persistence**: PostgreSQL 16 is the primary store. All schema changes must be managed via immutable Flyway migrations (V1-V4).
